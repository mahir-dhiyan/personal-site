				...
				<article class="border border-border rounded-lg p-6">
					...
					<div class="mt-4 flex flex-wrap gap-1.5">
						{#each project.tags as tag}
							<span class="font-mono text-xs bg-border/50 px-2.5 py-1 rounded text-ink-muted"
								>{tag}</span
							>
						{/each}
					</div>
				...
				{#each projects.filter((p) => !p.featured) as project}
					<article class="border border-border rounded-lg p-5">
						...
						<div class="mt-3 flex flex-wrap gap-1">
							{#each project.tags as tag}
								<span class="font-mono text-xs bg-border/50 px-2 py-0.5 rounded text-ink-muted"
									>{tag}</span
								>
							{/each}
						</div>
